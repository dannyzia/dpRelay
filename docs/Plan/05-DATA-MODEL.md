...
| Field | Type | Required | Written by | Description |
|-------|------|----------|------------|-------------|
| (key) | string | yes | SmsReceiver | RTDB push key generated by the authenticator app |
| txn_id | string | yes | SmsReceiver | 10-character alphanumeric TxID parsed from the payment confirmation SMS body |
| amount_bdt | number | yes | SmsReceiver | Amount in paisa (integer). e.g. `50000` = ৳500 |
| provider | string | yes | SmsReceiver | Payment provider identifier: `bkash` or `nagad` |
| sender | string | yes | SmsReceiver | Shortcode sender field as received by Android (e.g. `bKash`, `16247`) |
| received_at | number | yes | SmsReceiver | Firebase server timestamp via `ServerValue.TIMESTAMP` |

#### Security
- Write: `auth != null && auth.token.role == 'authenticator'`
- Read: `false` (admin SDK bypasses this)
- The node is deleted by the Cloud Function after successful or terminal backend handling.
