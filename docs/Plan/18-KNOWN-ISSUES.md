...
| TD-17 | Payment SMS parser in `SmsReceiver.kt` is regex-based and may break if bKash or Nagad change the wording of confirmation SMS messages. | medium | Inspect device logcat for `payment SMS parse failed` warnings; update regex if needed | Any driver reports payment sent but subscription not activated within 5 minutes |
