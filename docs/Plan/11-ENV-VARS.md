...
| WEBHOOK_EXHAUSTION_DAMPING_SEC | no | 900 | Minimum seconds between exhaustion alerts for the SAME app (dead-receiver damping; default caps re-alerts at 4/hour per dead receiver). 0 disables damping. Any successful delivery re-arms instantly. | server |
