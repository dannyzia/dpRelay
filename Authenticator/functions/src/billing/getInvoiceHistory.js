...
exports.getInvoiceHistory = onCall(
  ...
  async (request) => {
    ...
    let txQuery;
    if (appId) {
      // Requires composite index: (appId ASC, status ASC, requested_at DESC)
      txQuery = db
      ...
      // Admin gets all approved transactions in range.
      // Requires composite index: (status ASC, requested_at DESC)
      txQuery = db
        .collection("transactions")
        .where("status", "==", "approved")
        .where("requested_at", ">=", start)
      ...
      // Requires composite index: (ownerUid ASC, status ASC, requested_at DESC)
      txQuery = db
        .collection("transactions")
        .where("ownerUid", "==", uid)
        .where("status", "==", "approved")
    ...
    let otpUsage = [];
    if (appId) {
      // Range query on single field uses the auto-created single-field index.
    ...
    if (appId) {
      // Requires composite index: (appId ASC, createdAt DESC) – already exists.
      ...
      // Requires composite index: (ownerUid ASC, createdAt DESC) – added in firestore.indexes.json.
      bulkCampaignsQuery = db
        .collection("bulk_campaigns")
        .where("ownerUid", "==", uid)
        .where("createdAt", ">=", start)
    ...
    logger.info(
      `[getInvoiceHistory] uid=${uid} appId=${appId || "all"} range=${start.toISOString()}–${end.toISOString()} ` +
